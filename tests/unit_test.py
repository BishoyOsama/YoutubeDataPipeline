#============mock_api_key fixture test===========
def test_api_key(api_key):
    assert api_key == "MOCK_KEY1234"

#========mock_channel_handle fixture test===========
def test_channel_handle(channel_handle):
    assert channel_handle == "MOCK_CHANNEL"

#============mock_postgres_connection fixture test===========
def test_postgres_connection(mock_postgres_connection):
    conn = mock_postgres_connection
    assert conn.login == "mock_username"
    assert conn.password == "mock_password"
    assert conn.host == "mock_host"
    assert conn.port == 1234
    assert conn.schema == "mock_db_name"


def test_dags_integrity(dagbag):
    #1- check import errors
    assert dagbag.import_errors == {}, f"Import errors found: {dagbag.import_errors}"
    print("-----------------------------")
    print(dagbag.import_errors)

    #2- check expected number of DAGs
    expected_dag_ids = ["produce_json", "update_datawarehouse", "data_quality_checks"]
    loaded_dag_ids = list(dagbag.dags.keys())
    print("----------------------------------")
    print(dagbag.dags.keys())

    for dag_id in expected_dag_ids:
        assert dag_id in loaded_dag_ids, f"DAG '{dag_id}' is missing in the DagBag"

    #3- check how many DAGs are loaded
    assert dagbag.size() == 3, f"Expected 3 DAGs, but found {dagbag.size()}"
    print("---------------------------------")
    print(dagbag.size())

    #4- check each DAG has correct number of tasks
    expected_task_counts = {
        "produce_json": 5,
        "update_datawarehouse": 3,
        "data_quality_checks": 2
    }
    print("---------------------------------")
    for dag_id, dag in dagbag.dags.items():
        expected_count = expected_task_counts[dag_id]
        actual_count = len(dag.tasks)
        assert expected_count == actual_count, f"DAG '{dag_id}' expected {expected_count} tasks, but found {actual_count}"
        print(dag_id, len(dag.tasks))

